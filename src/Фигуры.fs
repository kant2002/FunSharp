...
type internal ИнфоФигуры = { Фигура:Shape; mutable Смещение:Point; mutable Opacity:float }
...
type Фигуры private () =
   ...
   static let шрифт () = 
      Font(ГрафическоеОкно.РазмерШрифта,ГрафическоеОкно.ИмяШрифта,ГрафическоеОкно.ЖирностьШрифта, ГрафическоеОкно.КурсивностьШрифта)
   ...
   static let добавитьФигуру название фигура =
      ...
      addDrawing (DrawShape(название,фигура))
   ...
   static member ДобавитьЛинию(x1,y1,x2,y2) =
      let имя = генИмя "Line"
      LineShape(Line(x1,y1,x2,y2),перо()) |> добавитьФигуру имя
      имя
   ...
   static member ДобавитьПрямоугольник(ширина,высота) =
      let имя = генИмя "Rectangle"
      RectShape(Rect(ширина,высота),перо(),кисть()) |> добавитьФигуру имя
      имя
   ...
   static member ДобавитьТреугольник(x1,y1,x2,y2,x3,y3) =
      let имя = генИмя "Triangle"
      TriangleShape(Triangle(x1,y1,x2,y2,x3,y3),перо(),кисть()) |> добавитьФигуру имя
      имя
   static member ДобавитьЭллипс(ширина,высота) =
      let имя = генИмя "Ellipse"
      EllipseShape(Ellipse(ширина,высота),перо(),кисть()) |> добавитьФигуру имя
      имя
   ...
   static member ДобавитьИзображение(имяИзображения) =
      ...
      | Some байты ->
         ...
         ImageShape(ref изображение) |> добавитьФигуру имя
      | None ->
         let изображениеСсыл = 
            ...
            then
               ...
               async {
                  let! изображение = Http.LoadImageAsync имяИзображения
               ...
               изображениеСсыл
         ImageShape(изображениеСсыл) |> добавитьФигуру имя
      имя
   static member ДобавитьТекст(текст) =
      let name = генИмя "Text"
      TextShape(ref текст, шрифт(), кисть()) |> добавитьФигуру name
      name
