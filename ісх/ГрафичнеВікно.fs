   ...
   статичний член НамалюватиЗображення(імяЗображення,x,y) =
      нехай посиланняЗображення =
         ...
         | Some байти -> 
            ...
            ссил (новий Bitmap(потокПамяти) :> Avalonia.Media.IImage)           
         | None ->
            якщо імяЗображення.StartsWith("http:") || імяЗображення.StartsWith("https:") 
            тоді
                нехай посиланняЗображення = ссил нуль
                асинх {
                ...
                посиланняЗображення
            інакше
                ссил (новий Bitmap(Reflection.Assembly.GetEntryAssembly().GetManifestResourceStream(імяЗображення)) :> IImage)
