...
type internal ИнфоФигуры = { Фигура:Фігура; mutable Зсув:Point; mutable Opacity:float }
...
type Фігури private () =
   ...
   static member ДобавитьЛинию(x1,y1,x2,y2) =
      ...
      ФігураЛінії(Лінія(x1,y1,x2,y2),перо()) |> добавитьФигуру имя
      имя
   ...
   static member ДодатиПрямокутник(ширина,высота) =
      ...
      ФігураПрямокутника(Прямокутник(ширина,высота),перо(),кисть()) |> добавитьФигуру имя
      имя
   ...
   static member ДодатиТрикутник(x1,y1,x2,y2,x3,y3) =
      ...
      ФігураТрикутника(Трикутник(x1,y1,x2,y2,x3,y3),перо(),кисть()) |> добавитьФигуру имя
      имя
   static member ДобавитьЭллипс(ширина,высота) =
      let имя = генИмя "Эллипс"
      ФігураЕлліпса(Елліпс(ширина,высота),перо(),кисть()) |> добавитьФигуру имя
      имя
   ...
   static member ДодатиЗображення(імяЗображення) =
      ...
      | Some байты ->
         ...
         ФігураЗображения(ref зображення) |> добавитьФигуру имя
      | None ->
         let зображенняПосил = 
            ...
            then
               ...
               зображенняПосил
         ФігураЗображения(зображенняПосил) |> добавитьФигуру имя
      имя
   static member ДобавитьТекст(текст) =
      let name = генИмя "Text"
      ФігураТекста(ref текст, шрифт(), кисть()) |> добавитьФигуру name
      name
   static member СкрытьФигуру(имяФигуры) =      
      Моя.Апплікація.Викликати (fun () -> Моя.Апплікація.Полотно.ВстановитиВидимістьФігури(имяФигуры,false))      
   static member ПоказатьФигуру(имяФигуры) =
      Моя.Апплікація.Викликати (fun () -> Моя.Апплікація.Полотно.ВстановитиВидимістьФігури(имяФигуры,true))      
   static member Перемістити(имяФигуры,x,y) =
      onShape имяФигуры (fun инфо ->
         инфо.Зсув <- Point(x,y)
         Моя.Апплікація.Викликати (fun () -> Моя.Апплікація.Полотно.ПереміститиФигуру(имяФигуры,инфо.Зсув))
      )
   ...
   static member SetOpacity(имяФигуры, opacity) =
      onShape имяФигуры (fun info ->
         ...
         Моя.Апплікація.Викликати (fun () -> Моя.Апплікація.Полотно.ВстановитиНепрозорістьФігури(имяФигуры,opacity))
      )
   ...
   static member Повернути(имяФигуры, угол) =
      match фигуры.TryGetValue(имяФигуры) with
      | true, info ->
         Моя.Апплікація.Викликати (fun () -> Моя.Апплікація.Полотно.ВстановитиОбертанняФігури(имяФигуры,угол))
      | false, _ -> ()
   ...
   static member УстановитьТекст(имяФигуры, текст) =
      onShape имяФигуры (fun info ->
         ...
         | ФігураТекста(textRef, font, color) ->
