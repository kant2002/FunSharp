...
type Черепаха private () =
   ...
   static let показать () =
      if not скрытоПользователем then
        Моя.Апплікація.Полотно.Черепаха.Видим <- true
   ...
   static member Угол
      ...
      and set значение = 
         угол <- значение
         Моя.Апплікація.Полотно.Черепаха.Врашение <- Some угол
         показать ()
   static member X
      with get () = _x
      and set значение = 
   ...
   static member Перемістити(відстань:float) =
      ...
      _x <- x'
      _y <- y'
      Моя.Апплікація.Полотно.Черепаха.Зсув <- Xwt.Point(_x,_y)
      показать ()
   ...
   static member Скрыть() =
      ...
      Моя.Апплікація.Полотно.Черепаха.Видим <- false
