...
type Черепаха private () =
   ...
   static let показать () =
      if not скрытоПользователем then
        Мое.Приложение.Полотно.Черепаха.Видим <- true
        Мое.Приложение.Полотно.СделатьНедействительным()
   ...
   static member Угол
      ...
      and set значение = 
         угол <- значение
         Мое.Приложение.Полотно.Черепаха.Врашение <- Some угол
         показать ()
   static member X
      with get () = _x
      and set значение = 
   ...
   static member Перемістити(відстань:float) =
      ...
      _x <- x'
      _y <- y'
      Мое.Приложение.Полотно.Черепаха.Зсув <- Xwt.Point(_x,_y)
      показать ()
   ...
   static member Скрыть() =
      ...
      Мое.Приложение.Полотно.Черепаха.Видим <- false
