...
let инкостровx = Словарь<int,int>()
let инкостровy = Словарь<int,int>()
...
let rec Инициализация() =
   ...
   ГрафическоеОкно.ЦветФона <- Цвета.Серый
   ГрафическоеОкно.ЦветПера <- Цвета.Желтый
...
and переместитьвсе () =
   ...
   for i=1 to количествоостровов do
      if ((позy.[i]+инкостровy.[i]) > (высотаИгры+15)) then // relaunch island if no more visible
         let R = int (Математика.Округлить(float (Математика.ВзятьСлучайноеЧисло(клвостровов))))
   ...
   while i <= количество_Врагов do
      ...
      if (Математика.ВзятьСлучайноеЧисло(Враг_Агрессивность)=1) then
