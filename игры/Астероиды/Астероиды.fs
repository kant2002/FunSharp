...
and ИзменитьНаправление () =   
   if (ГрафическоеОкно.ПоследняяКнопка = кнопкаПраво) then
     ...
     пауза <- Math.Remainder(пауза + 1, 2)  
   Фигуры.Повернуть(игрок, уголИгрока)

// Move all on screen items
and Движение  () =
   // Move player
   let x = Math.Remainder(Фигуры.ПолучитьЛево(игрок) + (Math.Cos(Math.GetRadians(уголИгрока - 90.0)) * скоростьИгрока) + ширинаИгры, ширинаИгры)
   let y = Math.Remainder(Фигуры.ПолучитьВерх(игрок) + (Math.Sin(Math.GetRadians(уголИгрока - 90.0)) * скоростьИгрока) + высотаИгры, высотаИгры)
   Фигуры.Переместить(игрок, x, y)

   // Move rocks
   for i = 0 to количествоАстероидов-1 do
     let x = Math.Remainder(Фигуры.ПолучитьЛево(астероид.[i]) + (Math.Cos(Math.GetRadians(уголАстероида.[i] - 90.0)) * скоростьАстероидов) + ширинаИгры, ширинаИгры)
     let y = Math.Remainder(Фигуры.ПолучитьВерх(астероид.[i]) + (Math.Sin(Math.GetRadians(уголАстероида.[i] - 90.0)) * скоростьАстероидов) + высотаИгры, высотаИгры)
     Фигуры.Переместить(астероид.[i], x, y)

   // Move ammo
   for i = 0 to количествоПуль-1 do
     let x = Math.Remainder(Фигуры.ПолучитьЛево(пуля.[i]) + (Math.Cos(Math.GetRadians(уголПули.[i] - 90.0)) * скоростьПули) + ширинаИгры, ширинаИгры)
     let y = Math.Remainder(Фигуры.ПолучитьВерх(пуля.[i]) + (Math.Sin(Math.GetRadians(уголПули.[i] - 90.0)) * скоростьПули) + высотаИгры, высотаИгры)
...
and ПроверкаСтолкновений () =
   ...
   px1 <- Фигуры.ПолучитьЛево(игрок) - ( (Math.Abs(ширинаИгрока * Math.Cos(Math.GetRadians(уголИгрока)) + высотаИгрока * Math.Sin(Math.GetRadians(уголИгрока))) - ширинаИгрока) / 2.0)
   py1 <- Фигуры.ПолучитьВерх(игрок) - ( (Math.Abs(ширинаИгрока * Math.Sin(Math.GetRadians(уголИгрока)) + высотаИгрока * Math.Cos(Math.GetRadians(уголИгрока))) - высотаИгрока) / 2.0)
   px2 <- px1 + Math.Abs(ширинаИгрока * Math.Cos(Math.GetRadians(уголИгрока)) + высотаИгрока * Math.Sin(Math.GetRadians(уголИгрока)))
   py2 <- py1 + Math.Abs(ширинаИгрока * Math.Sin(Math.GetRadians(уголИгрока)) + высотаИгрока * Math.Cos(Math.GetRadians(уголИгрока)))
...
and ДобавитьАстероид () =
   ...
   let размер,x,y =
      if (следующийРазмер <> 0) then
         ...
         let размер = минАстероид * Math.GetRandomNumber(типыАстероидов)
         let x = Math.GetRandomNumber(int ширинаИгры - размер)
         let y = Math.GetRandomNumber(int высотаИгры - размер)
   ...
   уголАстероида.Add(float (Math.GetRandomNumber(360)))
...
and УдалитьАстероид следующийУдаляемый =
   ...
   if (удаляемыйРазмер > 1) then
     ...
     while (удаляемыйРазмер > 0) do
       следующийРазмер <- Math.GetRandomNumber(удаляемыйРазмер - 1)
