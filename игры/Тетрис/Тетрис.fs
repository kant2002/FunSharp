...
and СкопироватьКусок () = // in basetemplate, template, rotation 
  ...
  if not (шаблоны.ContainsKey шаблон) then
      шаблоны.[шаблон] <-
         { Значения=[|0;0;0;0|]; Цвет=Цвета.Черный; Разм=0; ViewX=0; ViewY=0 }        
...
// GraphicsWindow.DrawResizedImage( Flickr.GetRandomPicture( "bricks" ), 0, 0, GraphicsWindow.Width, GraphicsWindow.Height)
  ...
  ГрафическоеОкно.ШиринаПера <- 1.0
  ГрафическоеОкно.ЦветПера <- Цвета.Розовый
  ...
  ГрафическоеОкно.ЦветПера <- Цвета.Черный
  ГрафическоеОкно.ЦветКисти <- Цвета.Розовый
...
and НастроитьШаблоны () =
  ...
  //_X_
  //_XX
  шаблоны.["template1"] <- { Значения=[|10;11;12;22|]; Цвет=Цвета.Желтый; Разм=3; ViewX = -12; ViewY = 12 }
  ...
  //XXX
  //_
  шаблоны.["template3"] <- { Значения=[|10;01;11;21|]; Цвет=Цвета.Серый; Разм=3; ViewX=0; ViewY=25}
  ...
  //_XX
  //_
  шаблоны.["template5"] <- { Значения=[|00;10;11;21|]; Цвет=Цвета.Зеленый; Разм=3; ViewX=0; ViewY=25 }

  //_XX
  //XX_
  //_
  шаблоны.["template6"] <- { Значения=[|10;20;01;11|]; Цвет=Цвета.Синий; Разм=3; ViewX=0; ViewY=25}

  //_X
  //_X
  //_X
  //_X
  шаблоны.["template7"] <- { Значения=[|10;11;12;13|]; Цвет=Цвета.Красный; Разм=4; ViewX=0; ViewY=0}
