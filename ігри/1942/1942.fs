...
ГрафичнеВікно.Висота <- высотаГри
...
let остров4 = СписокЗображень.ЗавантажитиЗображення(Путь + "island4.png")
let остров1= СписокЗображень.ЗавантажитиЗображення(Путь + "island1.png")
let остров2= СписокЗображень.ЗавантажитиЗображення(Путь + "island2.png")
let остров3= СписокЗображень.ЗавантажитиЗображення(Путь + "island3.png")
let остров5= СписокЗображень.ЗавантажитиЗображення(Путь + "island5.png")
let игрок0 = СписокЗображень.ЗавантажитиЗображення(Путь + "myplane1.png")
let background = СписокЗображень.ЗавантажитиЗображення(Путь + "fond.png")
let пуля0 = СписокЗображень.ЗавантажитиЗображення(Путь + "bullet.png")
let враг= СписокЗображень.ЗавантажитиЗображення(Путь + "enemy1.png")
let враг2 = СписокЗображень.ЗавантажитиЗображення(Путь + "enemy2.png")
let взрв_врага1=СписокЗображень.ЗавантажитиЗображення(Путь + "explo1.png")
let взрв_врага2=СписокЗображень.ЗавантажитиЗображення(Путь + "explo2.png")
let взрв_игрока=СписокЗображень.ЗавантажитиЗображення(Путь + "explo2.png")
let Enemy_bullet=СписокЗображень.ЗавантажитиЗображення(Путь + "E_bullet.png")
let ``конец`` = СписокЗображень.ЗавантажитиЗображення(Путь + "End.png")
...
let rec Инициализация() =
   ...
   фон <- Фігури.ДодатиЗображення(background)
   массив_Островов.[1] <- Фігури.ДодатиЗображення(остров1)
   массив_Островов.[2] <- Фігури.ДодатиЗображення(остров2)
   массив_Островов.[3] <- Фігури.ДодатиЗображення(остров3)
   массив_Островов.[4] <- Фігури.ДодатиЗображення(остров4)
   массив_Островов.[5] <- Фігури.ДодатиЗображення(остров5)
   for i = 6 to количествоостровов do
      массив_Островов.[i] <- массив_Островов.[Математика.Залишок(i,4)]

   ігрок <- Фігури.ДодатиЗображення(игрок0)
   ...
   while (гра = 1) do
      ...
      if (пауза = 0) then
         ...
         if (ВремяИгры > 4000) then
            Фігури.Перемістити(Фігури.ДодатиЗображення(``конец``), 230,200)
         ...
         ГрафичнеВікно.МишаДонизу <- Callback(НаНажатиеМыши)
...
and переместитьвсе () =
   ...
   for i = 1 to Игрок_КоличествоПуль do
      ...
      let Пуля_Игрокаx = Фігури.ОтриматиЛіво(имяФигуры) + инкпx
      let Пуля_Игрокаy = Фігури.ОтриматиВерх(имяФигуры) - СкоростьПули_Игрока
   ...
   for iea = 1 to КоличествоПуль_Врага do
      ...
      let Пуля_Врагаx = Фігури.ОтриматиЛіво(Пули_Врага.[iea]) + dx + инкпx
      let Пуля_Врагаy = Фігури.ОтриматиВерх(Пули_Врага.[iea]) + dy + инкпy * 0.1
...
and Огонь () =
   lock пулиСинхр (fun () -> 
      ...
      // Add the player Ammo
      Игрок_КоличествоПуль <- Игрок_КоличествоПуль + 1
      Пули_Игрока.[Игрок_КоличествоПуль] <- Фігури.ДодатиЗображення(пуля0)
...
and выстрел_Врага () =
   ...
   // Add the Enemy Ammo
   КоличествоПуль_Врага <- КоличествоПуль_Врага + 1
   Пули_Врага.[КоличествоПуль_Врага] <- Фігури.ДодатиЗображення(Enemy_bullet)
...
and создать_врагов_слева () =
   ...
   for i=1 to ном_Врага do
      ...
      if (тип_врага = 2) then
         массив_Врагов.[количество_Врагов] <- Фігури.ДодатиЗображення(враг2)
      else
         массив_Врагов.[количество_Врагов] <- Фігури.ДодатиЗображення(враг)
...
and Столкновение_ипв () =  // для пуль-игрока и врагов
   ...
   while i1 <= Игрок_КоличествоПуль do
      ...
      let Player_Ammox = int (Фігури.ОтриматиЛіво(shapeName))
      let Player_Ammoy = int (Фігури.ОтриматиВерх(shapeName))
      ...
      while i2 <= количество_Врагов do
         ...
         if ( (ax1 < px1 && ax2 > px1) || (ax1 < px2 && ax2 > px2) ) then
            if ( (ay1 < py1 && ay2 > py1) || (ay1 < py2 && ay2 > py2) ) then
               ...
               let toto = Фігури.ДодатиЗображення(взрв_врага1)
               Фігури.Перемістити(toto,ax1,ay1)
               Програма.Затримка(30)
               Фігури.Видалити(toto)
               let toto = Фігури.ДодатиЗображення(взрв_врага2)
               Фігури.Перемістити(toto,ax1,ay1)
               Програма.Затримка(30)
               Фігури.Видалити(toto)
               счет <- счет + 100
...
and Столкновения_ви () =   // для врагов и игрока 
   ...
   while i2 <= количество_Врагов do
      ...
      if ( (px1 < ax1 && px2 > ax1) || (px1 < ax2 && px2 > ax2) ) then
         if ( (py1 < ay1 && py2 > ay1) || (py1 < ay2 && py2 > ay2) ) then
            ...
            Програма.Затримка(30)
            Фігури.Видалити(toto)
            let toto = Фігури.ДодатиЗображення(взрв_игрока)
            Фігури.Перемістити(toto,ax1,ay1)
   ...
   while i3 <= КоличествоПуль_Врага do
      ...
      let ax1=int (Фігури.ОтриматиЛіво(Пули_Врага.[i3]))
      let ay1=int (Фігури.ОтриматиВерх(Пули_Врага.[i3]))
