...
ГрафичнеВікно.Ширина <- ширинаИгры
ГрафичнеВікно.Высота <- высотаИгры
...
and Игра () =
   ...
   for i=1 to количествоостровов do
      ...
      инкостровy.[i] <- 0   
   ВремяИгры <- 0
   Фигуры.Перемістити(игрок, ширинаИгры/2 , высотаИгры - 80 )
   while (игра = 1) do
      ...
      if (пауза = 0) then
         ...
         if (ВремяИгры > 4000) then
            Фигуры.Перемістити(Фигуры.ДобавитьИзображение(``конец``), 230,200)
         ...
         ГрафичнеВікно.МышьНажата <- Callback(НаНажатиеМыши)
...
and НаДвижениеМыши () =
   ...
   if (ракеткаX < 0) then
      ракеткаX <- 0   
   Фигуры.Перемістити(игрок, ракеткаX , ракеткаY )
...
and переместитьвсе () =
   инкпx <- 0.0
   инкпy <- 0.0
   ГрафичнеВікно.Заголовок <- заголовокИгры + счет.ToString() + " Жизни:" + Жизни_Игрока.ToString()
   ...
   if (ракеткаX < 64 ) then
      ...
      if (инкx = 32 ) then
         инкx <- 0
   Фигуры.Перемістити(фон,инкx - 32 ,инкy - 32)

   for i=1 to количествоостровов do
      let острx = позx.[i]+инкостровx.[i]
      let острy = позy.[i]+инкостровy.[i]
      Фигуры.Перемістити(массив_Островов.[i],острx,острy)
   ...
   for iea = 1 to КоличествоПуль_Врага do
      ...
      let Пуля_Врагаx = Фигуры.ПолучитьЛево(Пули_Врага.[iea]) + dx + инкпx
      let Пуля_Врагаy = Фигуры.ПолучитьВерх(Пули_Врага.[iea]) + dy + инкпy * 0.1
      Фигуры.Перемістити(Пули_Врага.[iea], Пуля_Врагаx, Пуля_Врагаy)
      let старыйВозраст =
   ...
   while i <= количество_Врагов do
      ...
      if (Время=etl) then  //время повернуть врага
         ...
         Фигуры.Повернути(массив_Врагов.[i],rr)
      if (Время > etl)  then
         uu <- uu+1
      ...
      Фигуры.Перемістити(массив_Врагов.[i],xx,yy)
      позX_Врага.[i] <- int xx
      позY_Врага.[i] <- int yy
...
and УдалитьПулю_Игрока (player_Ammo_nextRemove) =
   Фигуры.Видалити(Пули_Игрока.[player_Ammo_nextRemove])
...
and УдалитьПулю_Врага (enemy_Ammo_nextRemove) =
   Фигуры.Видалити(Пули_Врага.[enemy_Ammo_nextRemove])
...
and Огонь () =
   lock пулиСинхр (fun () -> 
      ...
      Фигуры.Перемістити(Пули_Игрока.[Игрок_КоличествоПуль], ВыстрелX, ВыстрелY)
   )
...
and выстрел_Врага () =
   ...
   Фигуры.Перемістити(shape, Выстрел_ВрагаX, Выстрел_ВрагаY)
...
and удалить_врага (next_enemy_remove) =
   Фигуры.Видалити(массив_Врагов.[next_enemy_remove])
...
and создать_врагов_слева () =
   ...
   for i=(количество_Врагов-ном_Врага+1) to количество_Врагов do
      ...
      Фигуры.Перемістити(массив_Врагов.[i],xxx,yyy)
      ...
      while i2 <= количество_Врагов do
         ...
         if ( (ax1 < px1 && ax2 > px1) || (ax1 < px2 && ax2 > px2) ) then
            if ( (ay1 < py1 && ay2 > py1) || (ay1 < py2 && ay2 > py2) ) then
               ...
               Фигуры.Перемістити(toto,ax1,ay1)
               Програма.Затримка(30)
               Фигуры.Видалити(toto)
   ...
   while i3 <= КоличествоПуль_Врага do
      ...
      if ( (px1 < ax1 && px2 > ax1) || (px1 < ax2 && px2 > ax2) ) then
         if ( (py1 < ay1 && py2 > ay1) || (py1 < ay2 && py2 > ay2) ) then
            ...
            // begin animation for explosion at coordinate ax1, ay1
            let toto = Фигуры.ДобавитьИзображение(взрв_врага1)
            Фигуры.Перемістити(toto,ракеткаX+ размер_игрока/2,ракеткаY+ размер_игрока/2)
            Програма.Затримка(30)
            Фигуры.Видалити(toto)
            let toto = Фигуры.ДобавитьИзображение(взрв_игрока)
            Фигуры.Перемістити(toto,ракеткаX+ размер_игрока/2,ракеткаY+ размер_игрока/2)
...
and ПозицииОстровов () =
   ...
   позОстровов.[1].[1] <- 1
   позОстровов.[1].[2] <- - int (Математика.Округляти(float ширинаИгры/2.0))
   позОстровов.[1].[3] <- -150
   позОстровов.[2] <- Словник()
   позОстровов.[2].[1] <- 2
   позОстровов.[2].[2] <- -2 * int (Математика.Округляти(float ширинаИгры/3.0))
   позОстровов.[2].[3] <- -150
   позОстровов.[3] <- Словник()
   позОстровов.[3].[1] <- 1
   позОстровов.[3].[2] <- 2 * int (Математика.Округляти(float ширинаИгры/3.0))
   позОстровов.[3].[3] <- -150
   ...
   позОстровов.[5].[1] <- 3
   позОстровов.[5].[2] <- int (Математика.Округляти(float ширинаИгры/3.0))
   позОстровов.[5].[3] <- -150
...
and создать_уровень1 () =  // this define the behavior of the différent squadron along the time play for level 1
   ...
   for i= 1 to 10 do
      ...
      уровень1.[7+i].[2] <- 6
      уровень1.[7+i].[3] <- Математика.ОтриматиВипадковеЧисло(ширинаИгры)
      уровень1.[7+i].[4] <- -50 + i
   ...
   for i= 1 to 10 do
     ...
     уровень1.[38+i].[2] <- 6
     уровень1.[38+i].[3] <- Математика.ОтриматиВипадковеЧисло(ширинаИгры)
