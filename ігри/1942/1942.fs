...
нехай ШвидкістьКули_Ігрока = плаваюча швф + 4.0
...
нехай позОстровів = Словник<цел,Словник<цел,цел>>()
нехай інкострівx = Словник<цел,цел>()
нехай інкострівy = Словник<цел,цел>()
нехай рівень1 = Словник<цел,Словник<цел,цел>>()
нехай шляхВорога = Словник<цел, Словник<цел, Словник<цел,плаваюча>>>()
нехай позx = Словник<цел,_>()
нехай позy = Словник<цел,_>()
...
та НаРухМиші () =
   ракеткаX <- цел ГрафичнеВікно.МишаX - розмір_ігрока / 2
   ракеткаY <- цел ГрафичнеВікно.МишаY - розмір_ігрока
...
та переміститиусе () =
   ...
   для i=1 до кількістьостровів зробити
      якщо ((позy.[i]+інкострівy.[i]) > (висотаГри+15)) тоді // relaunch island if no more visible
         нехай R = цел (Математика.Округляти(плаваюча (Математика.ОтриматиВипадковеЧисло(клкостровів))))
   ...
   для iea = 1 до КількістьКуль_Ворога зробити
      нехай dx = (Математика.Син((Кут_Куль_Ворога.[iea] )) * плаваюча Швидкість_КульВорога)
      нехай dy = (Математика.Кос((Кут_Куль_Ворога.[iea] )) * плаваюча Швидкість_КульВорога)
   ...
   доки i <= кількість_Ворога зробити
      ...
      нехай xx = плаваюча xx1+шляхВорога.[вNBR].[3].GetOrDefault(uu, 0.0)+інккx
      нехай yy = плаваюча yy1+шляхВорога.[вNBR].[4].GetOrDefault(uu, 0.0)+інккy*0.1
      ...
      позX_Ворога.[i] <- цел xx
      позY_Ворога.[i] <- цел yy
      час_Ворогів.[i] <- Час + 1.0
      якщо ((плаваюча uu > шляхВорога.[вNBR].[0].[0]) && (Час > etl)) тоді // if last timelife remove the enemy sprite 
...
та постріл_Ворога () =
   ...
   Кут_Куль_Ворога.[КількістьКуль_Ворога] <- Математика.АркТан(плаваюча(ракеткаX- Постріл_ВорогаX+розмір_ігрока/2)/плаваюча(ракеткаY-Постріл_ВорогаY))
...
та створити_ворогів_зліва () =
   ...
   для i=1 до ном_Ворога зробити
      ...
      Хронометраж1 <- Хронометраж1 - 64.0 / плаваюча швидкість_ворогів   
...
та створити_ворогів_справа () =
   ...
   для i=1 до ном_Ворога зробити
      ...
      позX_Ворога.[кількість_Ворога] <- позВорогаX1
      позY_Ворога.[кількість_Ворога] <- позВорогаY1
      час_Ворогів.[кількість_Ворога] <- плаваюча Хронометраж1
...
та Зіткнення_ипв () =  // для пуль-игрока и врагов
   ...
   доки i1 <= Гравець_КількістьКуль зробити
      ...
      нехай Player_Ammox = цел (Фігури.ОтриматиЛіво(назваФігури))
      нехай Player_Ammoy = цел (Фігури.ОтриматиВерх(назваФігури))
      ...
      нехай змінливий куліВидалені = ложь
...
та Зіткнення_ви () =   // для врагов и игрока 
   нехай px1 = цел (Фігури.ОтриматиЛіво(ігрок))
   нехай py1 = цел (Фігури.ОтриматиВерх(ігрок))
...
та ПозиціїОстровів () =
   ...
   позОстровів.[1].[1] <- 1
   позОстровів.[1].[2] <- - цел (Математика.Округляти(плаваюча ширинаГри/2.0))
   позОстровів.[1].[3] <- -150
   позОстровів.[2] <- Словник()
   позОстровів.[2].[1] <- 2
   позОстровів.[2].[2] <- -2 * цел (Математика.Округляти(плаваюча ширинаГри/3.0))
   позОстровів.[2].[3] <- -150
   позОстровів.[3] <- Словник()
   позОстровів.[3].[1] <- 1
   позОстровів.[3].[2] <- 2 * цел (Математика.Округляти(плаваюча ширинаГри/3.0))
   позОстровів.[3].[3] <- -150
   ...
   позОстровів.[5].[1] <- 3
   позОстровів.[5].[2] <- цел (Математика.Округляти(плаваюча ширинаГри/3.0))
   позОстровів.[5].[3] <- -150
