...
and СоздатьКусок () = // in: template ret: h
  ...
  h <- Текст.Додати("piece", количествоh)

  кускиКШаблонам.[h] <- шаблон

  ГрафичнеВікно.ШиринаПера <- 1.0
  ГрафичнеВікно.КолірПера <- Кольори.Black
  ГрафичнеВікно.КолірПензлика <- шаблоны.[шаблон].Цвет
...
// GraphicsWindow.DrawResizedImage( Flickr.GetRandomPicture( "bricks" ), 0, 0, GraphicsWindow.Width, GraphicsWindow.Height)
  ...
  ГрафичнеВікно.ЗаповнитиПрямокутник(СМЕЩЩЕНИЕX, СМЕЩЩЕНИЕY, ШИРИНАХ*ШИРИНАК, ВЫСОТАХ*ШИРИНАК)
...
and НапечататьСчет () =
  ...
  ГрафичнеВікно.КолірПензлика <- Кольори.Black
  ГрафичнеВікно.НарисоватьТекст(485, 70, Текст.Додати(Текст.ОтриматиЧастинуТекста( "00000000", 0, 8 - Текст.ОтриматиДовжину( string счет ) ), счет))
...
ГрафичнеВікно.КлавішаДонизу <- Callback(ОбработатьКнопки)
...
while true do
  ...
  ГрафичнеВікно.ПоказатиПовідомлення( "Игра завершена", "Small Basic Tetris" )
