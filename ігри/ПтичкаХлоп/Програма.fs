open Бiблiотека

ГрафичнеВікно.Ширина <- 288
ГрафичнеВікно.Высота <- 440

let фон = Фігури.ДобавитьИзображение("bg.png")
let земля = Фігури.ДобавитьИзображение("ground.png")
let чирикающая_птичка = Фігури.ДобавитьИзображение("bird_sing.png")
let труба1 = СписокЗображень.ЗагрузитьИзображение("tube1.png")
let труба2 = СписокЗображень.ЗагрузитьИзображение("tube2.png")
Фігури.Перемістити(земля, 0, 340)

/// Тип птицы
type Птица = { X:float; Y:float; VY:float; Жива:bool }
/// Respond to flap command
let хлоп (bird:Птица) = { bird with VY = - System.Math.PI }
/// Применяет гравитацию к птице
let гравитация (bird:Птица) = { bird with VY = bird.VY + 0.1 }
/// Applies physics to bird
let физика (bird:Птица) = { bird with Y = bird.Y + bird.VY }
/// Updates bird with gravity & physics
let обновить = гравитация >> физика
 
/// Формирует позиции труб уровня
let сгенерироватьУровень n =
   let rand = System.Random()
   [for i in 1..n -> 50+(i*150), 32+rand.Next(160)]

let уровень = сгенерироватьУровень 10

let трубы =
   [for (x,y) in уровень ->
      let труба1 = Фігури.ДобавитьИзображение(труба1)
      let труба2 = Фігури.ДобавитьИзображение(труба2)
      Фігури.Перемістити(труба1,x,y-320)
      Фігури.Перемістити(труба2,x,y+100)
      (x,y), труба1, труба2]

let скролл = ref 0
let хлопик = ref { X = 30.0; Y = 150.0; VY = 0.0; Жива=true }
let хлопменя () = if (хлопик.Value).Жива then хлопик.Value <- хлоп хлопик.Value

ГрафичнеВікно.КнопкаНатиснута <-    
    fun () -> if ГрафичнеВікно.ОстанняКнопка = "Space" then хлопменя ()
ГрафичнеВікно.МишаНатиснута <-
    fun () -> хлопменя ()

while true do
   хлопик := обновить !хлопик
   let птица = !хлопик
   Фігури.Перемістити(чирикающая_птичка, птица.X, птица.Y)
   for ((x,y),труба1,труба2) in трубы do
      Фігури.Перемістити(труба1, float (x + !скролл),float (y-320))
      Фігури.Перемістити(труба2, float (x + !скролл),float (y+100))
   Фігури.Перемістити(земля, !скролл % 48, 340)
   decr скролл  
   Програма.Затримка(20)
