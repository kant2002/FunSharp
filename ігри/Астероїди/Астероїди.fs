...
та Рух () =
   ...
   // Move rocks
   для i = 0 to кількістьАстероїдів-1 зробити
   ...
   // Move ammo
   для i = 0 to кількістьКуль-1 зробити
...
та ПеревіркаЗіткнень () =
   ...
   для i у астероїдиДоВидалення |> Seq.distinct |> List.ofSeq |> List.rev зробити
      ВидалитиАстероид i
   для j у кулиДоВидалення |> Seq.distinct |> List.ofSeq |> List.rev зробити
      ВидалитиПулю j
...
та ПеревіркаРівня () =
   якщо (кількістьАстероїдів < 1) тоді
     наступнийРозмір <- 0
     для i = 1 to іницАстероїди зробити
       ДодатиАстероїд()     
