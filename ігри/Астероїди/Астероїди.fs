...
нехай астероїд = ResizeArray()
нехай кутАстероїда = ResizeArray()
нехай размірАстероїда = ResizeArray()
нехай куля = ResizeArray()
нехай кутКулі = ResizeArray<float>()
нехай вікПулі = ResizeArray()
...
та ЗмінитиНапрямок () =   
   якщо (ГрафичнеВікно.ОстанняКнопка = кнопкаПраво) then
...
та Рух () =
   ...
   // Move rocks
   for i = 0 to кількістьАстероїдів-1 зробити
   ...
   // Move ammo
   for i = 0 to кількістьКуль-1 зробити
...
та ПеревіркаЗіткнень () =
   ...
   нехай астероїдиДоВидалення = ResizeArray()
   нехай кулиДоВидалення = ResizeArray()
   ...
   for i = 0 to кількістьАстероїдів-1 зробити
     ...
     // Player collison
     якщо (безпекаІгрока < 1) then
       якщо ( (ax1 < px1 && ax2 > px1) || (ax1 < px2 && ax2 > px2) ) then
         якщо ( (ay1 < py1 && ay2 > py1) || (ay1 < py2 && ay2 > py2) ) then
           КінецьГри()

     // Ammo collison
     for j in 0..кількістьКуль-1 зробити          
         нехай bx1 = Фігури.ОтриматиЛіво(куля.[j])
         нехай by1 = Фігури.ОтриматиВерх(куля.[j])
         нехай bx2 = bx1 + розмірКулі
         нехай by2 = by1 + розмірКулі

         якщо ( (ax1 < bx1 && ax2 > bx1) || (ax1 < bx2 && ax2 > bx2) ) then
            якщо ( (ay1 < by1 && ay2 > by1) || (ay1 < by2 && ay2 > by2) ) then           
               астероїдиДоВидалення.Add(i)
               кулиДоВидалення.Add(j)

   for i in астероїдиДоВидалення |> Seq.distinct |> List.ofSeq |> List.rev зробити
      ВидалитиАстероид i
   for j in кулиДоВидалення |> Seq.distinct |> List.ofSeq |> List.rev зробити
      ВидалитиПулю j

   // Decrease the time player is safe
   якщо (безпекаІгрока > 0) then
...
та ДодатиАстероїд () =
   ...
   нехай розмір,x,y =
      якщо (наступнийРозмір <> 0) then
         ...
         нехай x = Математика.ОтриматиВипадковеЧисло(int ширинаГри - розмір)
         нехай y = Математика.ОтриматиВипадковеЧисло(int высотаГри - розмір)
         розмір,float x,float y
   // Draw the rock
   ГрафичнеВікно.КолірПера <- колірАстероїдів
   нехай зображення =
      якщо розмір = 60 then великийАстероїд
...
та ПеревіркаРівня () =
   якщо (кількістьАстероїдів < 1) then
     наступнийРозмір <- 0
     for i = 1 to іницАстероїди зробити
       ДодатиАстероїд()     
