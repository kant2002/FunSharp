...
let большойАстероид = СписокЗображень.ЗавантажитиЗображення(путь + "Asteroids_BigRock.png")
let среднийАстероид = СписокЗображень.ЗавантажитиЗображення(путь + "Asteroids_MediumRock.png")
let малАстероид = СписокЗображень.ЗавантажитиЗображення(путь + "Asteroids_SmallRock.png")
let фон = СписокЗображень.ЗавантажитиЗображення(путь + "Asteroids_Sky.png")
...
let rec Инициализация () =
   ...
   ігрок <- Фігури.ДодатиЗображення(путь + "Asteroids_Ship.png")
   // player = Фігури.AddTriangle(playerWidth/2, 0, 0, playerHeight, playerWidth, playerHeight)
   Фігури.Перемістити(ігрок, (ширинаГри - ширинаИгрока) / 2.0, (float высотаГри - висотаІгрока) / 2.0)
   кутІгрока <- 0.0

// Главная игровая процедура
and Игра () =
   ГрафичнеВікно.Показати()
   ГрафичнеВікно.КлавішаДонизу <- Callback(ИзменитьНаправление)
...
and Движение  () =
   // Move player
   let x = Математика.Залишок(Фігури.ОтриматиЛіво(ігрок) + (Математика.Кос(Математика.ОтриматиРадіани(кутІгрока - 90.0)) * скоростьИгрока) + ширинаГри, ширинаГри)
   let y = Математика.Залишок(Фігури.ОтриматиВерх(ігрок) + (Математика.Син(Математика.ОтриматиРадіани(кутІгрока - 90.0)) * скоростьИгрока) + высотаГри, высотаГри)
   Фігури.Перемістити(ігрок, x, y)

   // Move rocks
   for i = 0 to количествоАстероидов-1 do
     let x = Математика.Залишок(Фігури.ОтриматиЛіво(астероид.[i]) + (Математика.Кос(Математика.ОтриматиРадіани(уголАстероида.[i] - 90.0)) * швидкістьАстероїдів) + ширинаГри, ширинаГри)
     let y = Математика.Залишок(Фігури.ОтриматиВерх(астероид.[i]) + (Математика.Син(Математика.ОтриматиРадіани(уголАстероида.[i] - 90.0)) * швидкістьАстероїдів) + высотаГри, высотаГри)
     Фігури.Перемістити(астероид.[i], x, y)

   // Move ammo
   for i = 0 to количествоПуль-1 do
     let x = Математика.Залишок(Фігури.ОтриматиЛіво(пуля.[i]) + (Математика.Кос(Математика.ОтриматиРадіани(уголПули.[i] - 90.0)) * швидкістьКулі) + ширинаГри, ширинаГри)
     let y = Математика.Залишок(Фігури.ОтриматиВерх(пуля.[i]) + (Математика.Син(Математика.ОтриматиРадіани(уголПули.[i] - 90.0)) * швидкістьКулі) + высотаГри, высотаГри)
...
and ПроверкаСтолкновений () =
   ...
   px1 <- Фігури.ОтриматиЛіво(ігрок) - ( (Математика.Модуль(ширинаИгрока * Математика.Кос(Математика.ОтриматиРадіани(кутІгрока)) + висотаІгрока * Математика.Син(Математика.ОтриматиРадіани(кутІгрока))) - ширинаИгрока) / 2.0)
   py1 <- Фігури.ОтриматиВерх(ігрок) - ( (Математика.Модуль(ширинаИгрока * Математика.Син(Математика.ОтриматиРадіани(кутІгрока)) + висотаІгрока * Математика.Кос(Математика.ОтриматиРадіани(кутІгрока))) - висотаІгрока) / 2.0)
   px2 <- px1 + Математика.Модуль(ширинаИгрока * Математика.Кос(Математика.ОтриматиРадіани(кутІгрока)) + висотаІгрока * Математика.Син(Математика.ОтриматиРадіани(кутІгрока)))
   py2 <- py1 + Математика.Модуль(ширинаИгрока * Математика.Син(Математика.ОтриматиРадіани(кутІгрока)) + висотаІгрока * Математика.Кос(Математика.ОтриматиРадіани(кутІгрока)))
   ...
   for i = 0 to количествоАстероидов-1 do
     let ax1 = Фігури.ОтриматиЛіво(астероид.[i])
     let ay1 = Фігури.ОтриматиВерх(астероид.[i])
     ...
     for j in 0..количествоПуль-1 do          
         let bx1 = Фігури.ОтриматиЛіво(пуля.[j])
         let by1 = Фігури.ОтриматиВерх(пуля.[j])
...
and ДобавитьАстероид () =
   ...
   let размер,x,y =
      if (следующийРазмер <> 0) then
         let размер = минАстероїд * следующийРазмер
         let x = Фігури.ОтриматиЛіво(следующаяПозиция)
         let y = Фігури.ОтриматиВерх(следующаяПозиция)
         следующийРазмер <- 0
         размер,x,y
      else
         // Choose a random size and position
         let размер = минАстероїд * Математика.ОтриматиВипадковеЧисло(типАстероїдів)
   ...
   астероид.Add(Фігури.ДодатиЗображення(зображення))
...
and ВидалитиАстероид следующийУдаляемый =
   let mutable удаляемыйРазмер = размерАстероида.[следующийУдаляемый] / минАстероїд
...
and Выстрел () =
   ...
   while (количествоПуль > (максКуль - 1)) do     
     ВидалитиПулю 0
   // Add the ammo
   ГрафичнеВікно.КолірПера <- колірКулі   
   пуля.Add(Фігури.ДобавитьЭллипс(розмірКулі, розмірКулі))
   Фігури.Перемістити(пуля.[количествоПуль], (px1 + px2 - розмірКулі) / 2.0, (py1 + py2 - розмірКулі) / 2.0)
...
// Check ammo age
and УстареваниеПуль () =
   while возрастПули.Count > 0 && (возрастПули.[0] > життяКулі) do     
      ВидалитиПулю 0
...
and КонецИгры () =
   ...
   ГрафичнеВікно.ПоказатиПовідомлення("Ваш счет " + (счет * мультипликаторОчков).ToString() + " очков. Спасибо за игру.", "Игра окончена!")
