...
and Движение  () =
   // Move player
   let x = Математика.Остаток(Фигуры.ПолучитьЛево(игрок) + (Математика.Кос(Математика.ВзятьРадианы(уголИгрока - 90.0)) * скоростьИгрока) + ширинаИгры, ширинаИгры)
   let y = Математика.Остаток(Фигуры.ПолучитьВерх(игрок) + (Математика.Син(Математика.ВзятьРадианы(уголИгрока - 90.0)) * скоростьИгрока) + высотаИгры, высотаИгры)
   Фигуры.Переместить(игрок, x, y)

   // Move rocks
   for i = 0 to количествоАстероидов-1 do
     let x = Математика.Остаток(Фигуры.ПолучитьЛево(астероид.[i]) + (Математика.Кос(Математика.ВзятьРадианы(уголАстероида.[i] - 90.0)) * скоростьАстероидов) + ширинаИгры, ширинаИгры)
     let y = Математика.Остаток(Фигуры.ПолучитьВерх(астероид.[i]) + (Математика.Син(Математика.ВзятьРадианы(уголАстероида.[i] - 90.0)) * скоростьАстероидов) + высотаИгры, высотаИгры)
     Фигуры.Переместить(астероид.[i], x, y)

   // Move ammo
   for i = 0 to количествоПуль-1 do
     let x = Математика.Остаток(Фигуры.ПолучитьЛево(пуля.[i]) + (Математика.Кос(Математика.ВзятьРадианы(уголПули.[i] - 90.0)) * скоростьПули) + ширинаИгры, ширинаИгры)
     let y = Математика.Остаток(Фигуры.ПолучитьВерх(пуля.[i]) + (Математика.Син(Математика.ВзятьРадианы(уголПули.[i] - 90.0)) * скоростьПули) + высотаИгры, высотаИгры)
...
and ДобавитьАстероид () =
   ...
   let размер,x,y =
      if (следующийРазмер <> 0) then
         ...
         let размер = минАстероид * Математика.ВзятьСлучайноеЧисло(типыАстероидов)
         let x = Математика.ВзятьСлучайноеЧисло(int ширинаИгры - размер)
         let y = Математика.ВзятьСлучайноеЧисло(int высотаИгры - размер)
