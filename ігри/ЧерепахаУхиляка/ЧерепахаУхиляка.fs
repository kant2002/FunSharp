...
та Готово () =
   ...
   for opacity in 100 .. -10 .. 0 зробити
...
та НаТик () =
   якщо not крутиться then
     крутиться <- true
     нехай пмс = Годинник.МинуліМілісекунди
     якщо пмс - останнійпмс > 500 then
       ДодатиОб'єкт()
       останнійпмс <- пмс    
     КрутитиОб'єкт()
     крутиться <- false
...
та КрутитиОб'єкт () =
   for i = iMin to iMax-1 зробити
     ...
     якщо д < (розмір.[об'єкти.[i].Род] + 16) / 2 then
