...
let mutable iMax = 0
/// Falling object
type Об'єкт = { mutable X:int; mutable Y:int; Род:int; НазваниеФигуры:string }
/// Falling objects
...
and КрутитиОб'єкт () =
   for i = iMin to iMax-1 do
     ...
     if y > гв then
       ...
       Фігури.Видалити(об'єкти.[i].НазваниеФигуры)
       iMin <- i + 1
     else
       Фігури.Перемістити(об'єкти.[i].НазваниеФигуры, x, y)
       об'єкти.[i].X <- x
       об'єкти.[i].Y <- y   
and ДодатиОб'єкт () =   
   iMax <- iMax + 1
   ...
   let y = -20
   об'єкти.Add({ X = x; Y=y; Род=род; НазваниеФигуры=названиеФигуры})
