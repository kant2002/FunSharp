...
and СкопіюватиШматок () = // in basetemplate, template, rotation 
  ...
  if not (шаблони.ContainsKey шаблон) then
      шаблони.[шаблон] <-
         { Значення=[|0;0;0;0|]; Колір=Кольори.Чорний; Розм=0; ViewX=0; ViewY=0 }        
...
// GraphicsWindow.DrawResizedImage( Flickr.GetRandomPicture( "bricks" ), 0, 0, GraphicsWindow.Width, GraphicsWindow.Height)
  ...
  ГрафичнеВікно.КолірПензлика <- Кольори.Чорний
...
and НалаштуватиШаблони () =
  ...
  //_X_
  //_XX
  шаблони.["template1"] <- { Значення=[|10;11;12;22|]; Колір=Кольори.Жовтий; Розм=3; ViewX = -12; ViewY = 12 }
  ...
  //XXX
  //_
  шаблони.["template3"] <- { Значення=[|10;01;11;21|]; Колір=Кольори.Сірий; Розм=3; ViewX=0; ViewY=25}
  ...
  //_XX
  //_
  шаблони.["template5"] <- { Значення=[|00;10;11;21|]; Колір=Кольори.Зелений; Розм=3; ViewX=0; ViewY=25 }

  //_XX
  //XX_
  //_
  шаблони.["template6"] <- { Значення=[|10;20;01;11|]; Колір=Кольори.Синій; Розм=3; ViewX=0; ViewY=25}

  //_X
  //_X
  //_X
  //_X
  шаблони.["template7"] <- { Значення=[|10;11;12;13|]; Колір=Кольори.Червоний; Розм=4; ViewX=0; ViewY=0}
