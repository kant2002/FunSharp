...
/// Named templates
нехай шаблони = Словник<строка,Шаблон>()
/// Array of box shape names
тип Квадрати () як this=
   успадкує ResizeArray<строка>()
   зробити для i = 0 до КВАДРАТИ-1 зробити this.Add("")
/// Piece name to boxes
нехай шматки = Словник<строка,Квадрати>()
/// Piece name to template name
нехай шматкиДоШаблонів = Словник<строка,строка>()
...
та СкопіюватиШматок () = // in basetemplate, template, rotation 
  ...
  якщо не (шаблони.ContainsKey шаблон) тоді
      шаблони.[шаблон] <-
  ...
  якщо обертання = "CW" тоді
    для i = 0 до КВАДРАТИ - 1 зробити // x' = y y' = L - 1 - x
      ...
      нехай y = (L - 1 - Математика.Floor(плаваюча v/10.0))
  ...
  інякщо обертання = "CCW" тоді
    для i = 0 до КВАДРАТИ - 1 зробити // x' = L - 1 - y y' = x
      ...
      нехай y = Математика.Floor(плаваюча v / 10.0)
      шаблони.[шаблон].Значення.[i] <- x * 10 + y
...
та ПереміститиШматок () = // in: ypos, xpos, h. ypos/xpos is 0-19, representing the top/left box coordinate of the piece on the canvas. h returned by CreatePiece
  для i = 0 до КВАДРАТИ - 1 зробити
    ...
    нехай x = Математика.Floor(плаваюча v / 10.0)
...
та ПеревіритиЗупинку () = // in: ypos, xpos, h ret: done
  ...
  якщо ``зроблено`` = 1 тоді
    для i = 0 до КВАДРАТИ - 1 зробити
      ...
      //x = Math.Floor(v/10)
      //y = Math.Remainder(v, 10)
      нехай x = (Математика.Floor(плаваюча v / 10.0) + xпоз)
      ...
      якщо x = ШИРИНАХ тоді
        ...
        для y1 = y downto 1 зробити
          для x1 = 0 до ШИРИНАХ - 1 зробити
            ...
            Фігури.Перемістити(piece, Фігури.ОтриматиЛіво(piece), Фігури.ОтриматиВерх(piece) + плаваюча ШИРИНАК)

  якщо лінійОчищено > 0 тоді
    рахунок <- рахунок + 100 * int (Математика.Округляти(плаваюча лінійОчищено * 2.15 - 1.0))
    НадрукуватиРахунок()
...
та НадрукуватиРахунок () =
  ...
  ГрафичнеВікно.КолірПензлика <- Кольори.Black
  ГрафичнеВікно.НамалюватиТекст(485, 70, Текст.Додати(Текст.ОтриматиЧастинуТекста( "00000000", 0, 8 - Текст.ОтриматиДовжину(строка рахунок ) ), рахунок))
