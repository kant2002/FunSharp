...
let инициализироватьОкно () =
   ...
   for angle in 1.0..180.0 do
     let x = СредX+(Радиус+15.)*Math.Cos(Math.ВзятьРадианы(angle))
     let y1 = СредY+Радиус*Math.Sin(Math.ВзятьРадианы(angle))+15.
     let y2 = СредY+(Радиус+15.)*Math.Sin(Math.ВзятьРадианы(-angle))+10.
     let базоваяЯркость = Math.ВзятьСлучайноеЧисло(40)+30
     ГрафическоеОкно.ШиринаПера <- Math.ВзятьСлучайноеЧисло(5) |> float
     let цвет = 
       ГрафическоеОкно.ПолучитьЦветИзRGB(
         базоваяЯркость+100+Math.ВзятьСлучайноеЧисло(10),
         базоваяЯркость+60+Math.ВзятьСлучайноеЧисло(20),
         базоваяЯркость)
   ...
   for i in 1. .. 12. do
     let Радианы = Math.ВзятьРадианы(-i * 30. + 90.)
   ...
   if (float Часы.Час + float Часы.Минута/60. + float Часы.Секунда/3600. <> Час) then
     ...
     ЧасоваяСтрелка <- 
       Фигуры.ДобавитьЛинию(
         СредX,
         СредY,
         СредX+Радиус/2.*Math.Cos(Math.ВзятьРадианы(Час*30.-90.)),
         СредY+Радиус/2.*Math.Sin(Math.ВзятьРадианы(Час*30.-90.)))   
   if float Часы.Минута <> Минута then
     ...
     МинутнаяСтрелка <- 
       Фигуры.ДобавитьЛинию(
         СредX,
         СредY,
         СредX+Радиус/1.2*Math.Cos(Math.ВзятьРадианы(Минута*6.-90.)),
         СредY+Радиус/1.2*Math.Sin(Math.ВзятьРадианы(Минута*6.-90.)))   
   if float Часы.Секунда <> Секунда then
     ...
     СекунднаяСтрелка <- 
       Фигуры.ДобавитьЛинию(
         СредX,
         СредY,
         СредX+Радиус*Math.Cos(Math.ВзятьРадианы(Секунда*6.-90.)),
         СредY+Радиус*Math.Sin(Math.ВзятьРадианы(Секунда*6.-90.)))
